...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    // The inner elements of the fake scrollbars
    var scrollbarHInner = elt("div", null, null, "height: 1px"), scrollbarVInner = elt("div", null, null, "width: 1px");
    // The actual fake scrollbars.
    var scrollbarH = elt("div", [scrollbarHInner], "CodeMirror-hscrollbar");
    var scrollbarV = elt("div", [scrollbarVInner], "CodeMirror-vscrollbar");
    ...
    var wrapper = elt("div", [gutters, inputDiv, scrollbarH, scrollbarV, scroller],
    ...
    connect(wrapper, "scroll", function() {wrapper.scrollLeft = wrapper.scrollTop = 0;});
    ...
    // FIXME keep lastScrollLeft, update line number positions when
    // needed
    function onScrollBarH() {
      scroller.scrollLeft = scrollbarH.scrollLeft;
    }
    function onScrollBarV() {
      if (scrollbarV.scrollTop != lastScrollTop) {
        lastScrollTop = scroller.scrollTop = scrollbarV.scrollTop;
        updateDisplay([]);
      }
    }
    ...
    function updateScrollbars(scrollTop) {
      ...
      var needsVScroll = scroller.scrollHeight > scroller.clientHeight;
      var needsHScroll = scroller.scrollWidth > scroller.clientWidth;
      if (needsVScroll) {
        scrollbarV.style.display = "block";
        scrollbarV.style.bottom = needsHScroll ? scrollbarWidth(measure) + "px" : "0";
        scrollbarVInner.style.height = (scroller.scrollHeight - wrapper.clientHeight + scrollbarV.clientHeight) + "px";
        if (scrollTop != null) {
      ...
      if (needsHScroll) {
        ...
        scrollbarH.style.right = needsVScroll ? scrollbarWidth(measure) + "px" : "0";
        // FIXME doesn't allow scrolling all the way to right
        scrollbarHInner.style.width = (scroller.scrollWidth - wrapper.clientWidth + scrollbarH.clientWidth) + "px";
    ...
    function calculateScrollPos(x1, y1, x2, y2) {
      var pt = paddingTop();
      y1 += pt; y2 += pt;
      var screen = wrapper.clientHeight, screentop = scroller.scrollTop, result = {};
      var docBottom = scroller.scrollHeight;
      var atTop = y1 < pt + 10, atBottom = y2 + pt > docBottom - 10;
      if (y1 < screentop) result.scrollTop = atTop ? 0 : Math.max(0, y1);
      else if (y2 > screentop + screen) result.scrollTop = (atBottom ? docBottom : y2) - screen;

      var screenw = wrapper.clientWidth, screenleft = scroller.scrollLeft;
    ...
    function measureLine(line, ch) {
      for (var i = 0; i < measureLineMemo.length; ++i) {
        ...
        if (memo.ch == ch && memo.text == line.text &&
            (!options.textWrapping || wrapper.clientWidth == memo.width))
    ...
    function endOperation() {
      ...
      if (maxLineChanged && !options.lineWrapping) {
        ...
        if (!ie_lt8)
          lineSpace.style.minWidth = left + "px";
  ...
  function scrollbarWidth(measure) {
    ...
    console.log("sw", knownScrollbarWidth);
